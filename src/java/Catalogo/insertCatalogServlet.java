...
public class insertCatalogServlet extends HttpServlet {
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
        ...
        try {
            ...
            ResultSet resultSet = statement.executeQuery("select * from catalogo where Email = '" + txtEmail + "'");
            if (resultSet.next()) {
                out.println("<script type='text/javascript'>alert('Email already used');</script>");
                RequestDispatcher rd = request.getRequestDispatcher("/index.html");
                rd.include(request, response);
            } else {
                String sql = "insert into catalogo (Name, Email, Phone, Marca, Modelo, Año, tipo, id, estado) "
                        + "values ('" + txtName + "', '" + txtEmail + "', " + txtPhone + ", '" + txtMarca + "', '" + txtModelo + "', " + txtyear + ", '" + txttipo + "', '" + getRandomNumber() + "', 'En Venta')";
